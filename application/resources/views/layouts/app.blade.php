        ...
        <aside class="sidebar" id="sidebar">
            ...
            <nav class="nav-section">
                ...
                @if (auth()->user()->isSupervisor())
                    ...
                    @endif
                    {{-- Every supervisor gets the calendar. What is due and when
                         is the question a supervisor answers all day, whichever
                         part of the shop is theirs — and the route already let
                         them in, so only the link was missing. --}}
                ...
                @elseif (auth()->user()->isLeader())
                    ...
                    {{-- A leader who also holds the order desk approves samples
                         as the account officer, and that is a different queue on
                         a different page. The route already admitted her; only
                         the sales block carried the link, so the work sat on a
                         page she had no way to reach from here. --}}
                ...
                {{-- The raw materials supervisor reads as an agent, so she misses
                     the supervisor block above. Her calendar goes here. --}}
                @if (auth()->user()->canSeeCalendar() && ! auth()->user()->isSupervisor()
                     && ! auth()->user()->isLeader() && ! auth()->user()->isSales())
                    <a href="{{ route('calendar') }}" class="nav-item {{ request()->routeIs('calendar') ? 'active' : '' }}">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
                        Calendar
                    </a>
